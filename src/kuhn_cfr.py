...
class KuhnCFR:
    ...
    def __init__(self, iterations: int, decksize: int) -> None:
        ...
        #We have 2 actions available for the players: Pass(0) and Bet(1)
        self.bet_options: int = 2 
        self.nodes: Dict[str, Node] = {}

    #In this function: Run CFR algorithm for the specified number of iterations
    ...
    def external_cfr(
        ...
        if plays >= 2:
            if history[-1] == 0 and history[-2] == 1:  #1st scenario: Bet fold, whoever acts wins because the other folds
                if acting_player == traversing_player: #we are calculating the util of traversing_player
                    return 1.0 #if traversing_player is the acting_player, utility is positive
                else:
                    return -1.0
        ...
        infoset: str = str(cards[acting_player]) + str(history) #Creating infoset string
