...
class KuhnTest:
    ...
    def gameValue(self) -> float:
        ...
        card_distributions: List[List[int]] = [
            [1, 2],
            [1, 3],
            [2, 1],
            [2, 3],
            ...
            for next_move in ["p", "b"]:
                ...
                if isTerminal(history + next_move):
                    ...
                    for card in possibleCards:
                        ...
                        if curr_player == 0:
                            newRP[card + "0"] = (
                                reachProb[card + "0"] * currNode.getAverageStrategy()[a]
                            )
                            newRP[card + "1"] = reachProb[card + "1"]
                        else:
                            newRP[card + "0"] = reachProb[card + "0"]
                            newRP[card + "1"] = (
                                reachProb[card + "1"] * currNode.getAverageStrategy()[a]
                            )
                    gameTree = traverseRecursive(history + next_move, newRP, gameTree)
                    for card in possibleCards:
                        ev_temp = 0.0
                        if "br" not in gameTree[card + history]:
                            gameTree[card + history]["br"] = -gameTree[
                                card + history + next_move
                            ]["ev"]
                        else:
                            gameTree[card + history]["br"] = max(
                                gameTree[card + history]["br"],
                                -gameTree[card + history + next_move]["ev"],
                            )
                        npEV = 0.0
                        for other in childCards[card]:
                            ...
                            ev_temp += (
                                reachProb[evRP]
                                * currNode.getAverageStrategy()[a]
                                * -gameTree[card + history + next_move]["br"]
                            )
                        if npEV != 0:
                            ev_temp /= npEV
                        gameTree[card + history]["ev"] += ev_temp
